...
class SequenceAlignmentManager implements SequenceAlignmentInterface
{
    ...
    public function parseClustal()
    {
        ...
        $iLineCount    = $iMaxLength = $iGapCount = 0;
        ...
        foreach($aLines as $sLine) {
            ...
            // The conservation line closing each block leaves the name column blank, and
            // its symbols ("*", ":", ".") are spaced apart : read as words it would look
            // like a sequence named after its own first symbols.
            if (ctype_space(substr($sLine, 0, 1))) {
                continue;
            }
            ...
            foreach($aWords as $sWord) {
                // Trimming here also drops the "\r" a CRLF-terminated file leaves on the
                // last word: kept, it would be concatenated into the middle of a sequence.
                $sWord = trim($sWord);
                if($sWord != "") {
                    $aWordLines[] = $sWord;
                }
            ...
            // ClustalW closes each line with the number of residues written so far, where
            // Clustal Omega writes none. It counts what precedes it rather than adding to
            // it, so it is dropped : kept, the line would hold three words and be skipped
            // altogether, silently truncating every sequence of the alignment.
            if(sizeof($aWordLines) == 3 && is_numeric($aWordLines[2])) {
                array_pop($aWordLines);
            }
        ...
        foreach($aSequences as $sKey => $sSeqData) {
            ...
            if (strlen($sSeqData) > $iMaxLength) {
                $iMaxLength = strlen($sSeqData);
            }
